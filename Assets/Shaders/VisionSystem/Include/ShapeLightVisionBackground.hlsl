...
half4 CombinedShapeLightShared(half4 color, half4 mask, half2 lightingUV, half2 worldUV)
{
    ...
    // --- IŞIK KANALI 0 (Dim Light için kullanabilirsin) ---
    ...
    // --- IŞIK KANALI 3 (Spot Light / FOV için kullanacağız) ---
    ...
    // --- BURASI ASIL DEĞİŞİKLİK ---
    half4 finalOutput;
    #if !USE_SHAPE_LIGHT_TYPE_0 && !USE_SHAPE_LIGHT_TYPE_1 && !USE_SHAPE_LIGHT_TYPE_2 && ! USE_SHAPE_LIGHT_TYPE_3
        finalOutput = color; // Hiç ışık yoksa
    #else
        // 1. "Base" (Dim Light) aydınlatmasını hesapla (Kanal 0, 1, 2)
        half4 baseModulate = shapeLight0Modulate + shapeLight1Modulate + shapeLight2Modulate;
        half4 baseAdditive = shapeLight0Additive + shapeLight1Additive + shapeLight2Additive;
        half4 baseLightColor = (color * baseModulate + baseAdditive);

        // 2. "Spot" (FOV) aydınlatmasını hesapla (Sadece Kanal 3)
        half4 spotLightColor = (color * shapeLight3Modulate + shapeLight3Additive);

        // 3. İkisini TOPLAMAK yerine, en parlak olanını SEÇ (MAX)
        finalOutput = max(baseLightColor, spotLightColor);

        // HDR ve scale uygula
        finalOutput *= _HDREmulationScale;
    #endif
    // --- DEĞİŞİKLİK BİTTİ ---

    finalOutput.a = color.a;
    
    // Gri (gray) efekti kaldırıldı

    // Orijinal 'UseSceneLighting' mantığı kalıyor
